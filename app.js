...
function buildTeamList() {
    inquire.prompt(employeeQuestions).then(employeeInfo => {
        if (employeeInfo.role == "manager") {
            var newMember = new manager(employeeInfo.name, teamList.length + 1, employeeInfo.email, employeeInfo.github);
        }
...
function buildHtmlPage() {
    let newFile = fs.readFileSync("Develop/templates/main.html")
...
function buildHtmlCard(memberType, name, id, email, propertyValue) {
    let data = fs.readFileSync(`Develop/templates/${memberType}.html`, 'utf8')
